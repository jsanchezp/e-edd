package es.ucm.fdi.edd.ui.launch;
...
import es.ucm.fdi.edd.ui.wizards.EDDQuestionsWizard;
import es.ucm.fdi.edd.ui.wizards.MyWizardDialog;

public class EDDLaunchShortcut implements ILaunchShortcut {
	...
	private void doLaunch(final String mode, final Collection<IErlProject> projects) throws CoreException {
...
//		openWizard("es.ucm.fdi.edd.ui.wizards.EDDQuestionsWizard", new StructuredSelection(projects));
        
		EDDQuestionsWizard wizard = new EDDQuestionsWizard();
        wizard.init(PlatformUI.getWorkbench(), new StructuredSelection(projects));
        MyWizardDialog dialog = new MyWizardDialog(new Shell(), wizard);
        dialog.create();
        dialog.open();
    }
	
	private void openWizard(String id, StructuredSelection selection) {
		...
		try  {
			...
			if  (descriptor != null) {
				...
				((EDDQuestionsWizard) wizard).init(PlatformUI.getWorkbench(), selection);
    ...
    private ILaunchConfiguration getLaunchConfiguration(final Collection<IErlProject> projects, final String mode) throws CoreException {
        ...
        wc.setAttribute(ErlRuntimeAttributes.PROJECTS, ListsUtils.packList(projectNames, PROJECT_NAME_SEPARATOR));
        ...
        wc.setAttribute(ErlRuntimeAttributes.USE_LONG_NAME, !HostnameUtils.canUseShortNames()); // prefer short names
    ...
    private ILaunchConfiguration addInterpretedModules(final Collection<IErlProject> projects, final ILaunchConfiguration launchConfiguration) throws CoreException {
        final List<String> moduleNames = getProjectAndModuleNames(projects);
